    ...
    passport.use(
        ...
        new LocalStrategy({
            ...
            async (userName, userPassword, done) => {
                try {
                    let rows = await queryPromise('SELECT * FROM users WHERE userName = ? OR email=?',
                        [userName, userName]);
                    if (rows[0]) {
                ...
                } catch(err) { return done(false, 'Error... Try again later, please! ' + err.message); }
        })
    );
        ...
        (accessToken, refreshToken, profile, done) => {
            process.nextTick(async () => {
                try {
                    ...
                    if (rows[0]) {
                        await queryPromise('UPDATE facebook SET token=? WHERE fbId=?',
                            [accessToken, profile.id]);
                        return done({id: rows[0].userId, userName: profile.displayName});
                    }
                    if (profile._json.email)  {
                        rows = await queryPromise('SELECT * FROM users WHERE email=?',
                            [profile._json.email]);
